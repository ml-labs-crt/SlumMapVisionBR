...
df = df_new_boxes[df_new_boxes['location_ID']\
    .isin(df_locations['CD_GEOCODM'].to_list())]
...
df_all_scenes = pd.DataFrame(columns=['entityId', 'displayId', \
    'cloudCover', 'spatialCoverage',
       'spatialBounds', 'startDate', 'overlap', 'location'])
...
for i in range(0,len(df_for_payload)):
    ...
    if len(all_data['results']) > 0:
        ...
        for footprint in df_scenes["spatialCoverage"].tolist():
            s = shape(footprint)
            # footprints.append(s)
            overlap = 100.0*(my_location_shape.intersection(s).area\
                / my_location_shape.area)
        ...
        df_scenes = df_scenes.assign(location=len(df_scenes)\
            *[df_for_payload['CD_MUNICIP'].iloc[i]])

        df_scenes = pd.DataFrame(df_scenes)

        df_all_scenes = pd.concat([df_all_scenes,df_scenes], axis=0)

        print('Finished adding info #{} about {}. Scenes found {}.'\
        .format(i,df_for_payload['CD_MUNICIP'].iloc[i],len(df_scenes)))
    
    else:
        locations_no_scene.append(str(df_for_payload['CD_MUNICIP'].iloc[i]))
        print('No info #{} about {}.'\
        .format(i,df_for_payload['CD_MUNICIP'].iloc[i]))
