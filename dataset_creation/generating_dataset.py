...
for location in location_list:
    ...
    PATH_FOLDER = "D:/brazilian_dataset/2010_landsat/" + str(folder) + "/" + entityId
    ...
    (
        ...
        maxx_overlap_box,
        maxy_overlap_box,
    ) = overlap_region.bounds
    bbox_overlap_region = shapely.geometry.box(
        minx_overlap_box, miny_overlap_box, maxx_overlap_box, maxy_overlap_box
    )

    overlap = overlap_region.area / bbox_region.area
    print(
        "The location of interest {} is {}% in the raster.".format(
            location, round(overlap, 3) * 100
        )
    )

    geo_bbox_overlap_region = gpd.GeoDataFrame(
        {"geometry": bbox_overlap_region}, index=[0], crs=landsat_epsg
    )
    ...
    out_img_mask, out_transform_mask = mask(
        dataset=band_red_clipped, shapes=shapes_slums, crop=False
    )

    out_meta_mask = band_red_clipped.meta
    out_meta_mask.update({"count": 1, "transform": out_transform_mask, "nodata": 2.0})
    # print(out_meta_mask)
    ...
    out_img_mask_final = np.reshape(
        out_img_mask_mod, (1, out_img_mask_mod.shape[0], out_img_mask_mod.shape[1])
    )
