...
print('There are {} unique scenes.'\
    .format(len(df['entityId'].unique())))
...
print('{} scenes out of {} are available for download.'\
    .format(len(downloads),len(df['entityId'].unique())))
...
for i in range(0,len(downloads)):
    if len(downloadUrls) == len(downloads):
        ...
        location_download = df[df['entityId']==entity_to_download]\
            ['location'].iloc[0]
        response = requests.get(downloadUrls[i])
        PATH_TO_FILE = PATH_DOWNLOAD + "/" + str(location_download) +\
            "/" + entity_to_download + "/"
        pathlib.Path(PATH_TO_FILE).mkdir(parents=True, exist_ok=True)
        open(PATH_TO_FILE+"{}.tar".format(entity_to_download), "wb")\
